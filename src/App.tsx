 ...
 */
const App: React.FC = () => {
  // State for the uploaded image.  `imageSrc` holds the DataURL used to render
  // the preview; `originalImage` is an HTMLImageElement used as the source
  // for drawing onto the canvas.  Using an Image object avoids repeatedly
  // decoding the DataURL when applying filters.
  ...
  useEffect(() => {
    ...
    const process = async () => {
      ...
      if (data !== imageData.data) {
        const newImage = new ImageData(new Uint8ClampedArray(data), canvas.width, canvas.height);
